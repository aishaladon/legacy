...
const { buildProposalDocx } = require('../utils/buildProposalDocx');
...
router.post('/:id/download-proposal', async (req, res) => {
  ...
  try {
    const buffer = await buildProposalDocx(title, draft);
...
// Saves the RFP response draft as a real Draft in the info@legacypnp.ltd
// mailbox, with the same .docx used by Download as Word attached — so it's
// ready to address and send once you've picked a recipient, instead of
// only living as a file on disk or text in the app.
router.post('/:id/draft-proposal-save', async (req, res) => {
  const [[opp]] = await db.query('SELECT id, title FROM opportunities WHERE id = ?', [req.params.id]);
  if (!opp) return res.status(404).json({ error: 'Opportunity not found.' });

  const { draft } = req.body;
  if (!draft || !draft.trim()) {
    return res.status(400).json({ error: 'Nothing to save — the draft is empty.' });
  }

  try {
    const buffer = await buildProposalDocx(opp.title, draft);
    await saveDraftEmail({
      subject: `RFP Response — ${opp.title}`,
      body: `Draft RFP response for "${opp.title}" is attached.\n\nGenerated by Legacy GovCon on ${new Date().toLocaleDateString()}. Review before sending.`,
      attachments: [{
        filename: `RFP_Response_${opp.id}.docx`,
        content: buffer,
    ...
    console.error('Save proposal draft to mailbox error:', err);
